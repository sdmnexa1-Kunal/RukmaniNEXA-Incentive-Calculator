...
$('aImageFile').addEventListener('change',()=>{const f=$('aImageFile').files[0];if(!f){$('aImage').value='';$('aImagePreview').textContent='No creative selected.';return}if(!f.type.startsWith('image/')){status('announceStatus','Please select an image file.');$('aImageFile').value='';return}$('aImagePreview').textContent=`Selected: ${f.name}`});
$('announceBtn').onclick=async()=>{try{const title=$('aTitle').value.trim(),message=$('aMessage').value.trim();if(!title)throw Error('Enter a title.');const file=$('aImageFile').files[0];let imageUrl=$('aImage').value.trim()||null;if(file){if(!file.type.startsWith('image/'))throw Error('Please select an image file.');if(file.size>8*1024*1024)throw Error('Image must be 8 MB or smaller.');const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,'_');const path=`${Date.now()}-${safe}`;status('announceStatus','Uploading creative…');const {error:upErr}=await sb.storage.from('announcements').upload(path,file,{contentType:file.type,upsert:false});if(upErr)throw upErr;imageUrl=sb.storage.from('announcements').getPublicUrl(path).data.publicUrl;$('aImage').value=imageUrl;$('aImagePreview').textContent='Creative uploaded successfully.'}const starts_at=$('aStart').value?new Date($('aStart').value).toISOString():new Date().toISOString();const ends_at=$('aEnd').value?new Date($('aEnd').value).toISOString():null;const {error}=await sb.from('announcements').insert({title,message,image_url:imageUrl,starts_at,ends_at,display_frequency:$('aFreq').value,is_active:true});if(error)throw error;status('announceStatus','✅ Announcement published.');loadStatus()}catch(e){status('announceStatus','Error: '+e.message)}};
